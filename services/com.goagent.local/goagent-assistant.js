...
function handleGoagent(future,args){
	...
	this.file = "/media/cryptofs/apps/usr/palm/services/com.goagent.local/"
			+ "running.status";
	this.startScript = "/media/cryptofs/apps/usr/palm/services/com.goagent.local/"
			+ "scripts/start.sh";
	this.stopScript = "/media/cryptofs/apps/usr/palm/services/com.goagent.local/"
			+ "scripts/stop.sh";
	...
	if(args.state) {
		...
		if(this.state=="on") {
			this.fs.writeFile(this.file, JSON.stringify({state:"on"}),
					"utf8");
			this.cmd = new CommandLine("/bin/sh " + this.startScript,
					this.future);
			this.cmd.run();
